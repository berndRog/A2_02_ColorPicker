...
fun SelectColorScreen1(
   ...
   Column(
      ...
      ColorSlider(
         ...
         onValueChange = { it -> stateColor.value = Color(it, green, blue) }
      )
      ColorSlider(
         label = "G(rün)",
         value = stateColor.value.green,
         onValueChange = { it -> stateColor.value = Color(red, it, blue) }
      )
      ColorSlider(
         label = "B(lau)",
         value = stateColor.value.blue,
         onValueChange = { it -> stateColor.value = Color(red, green, it) }
      )
